...
module.exports = {
    ...
    run: async ({ interaction }) => {
        try {
            ...
            initEmbed.setDescription(`Es el turno de ${interaction.user} de elegir.`);
            ...
            initEmbed.setDescription(`${interaction.user} ha elegido ${userChoice.name} ${userChoice.emoji}\n${oponente} ha elegido ${targetUserChoice.name} ${targetUserChoice.emoji}`)
                .addFields({ name: 'Resultado', value: result === 'Empate' ? 'Ha ocurrido un empate' : result === 'Ganaste' ? `${interaction.user} ha ganado` : `${oponente} ha ganado` });
